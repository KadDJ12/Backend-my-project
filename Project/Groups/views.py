...
class GroupViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if user.role == 'teacher':
            ...
            group_ids = Lesson.objects.filter(
                teacher=user, group__isnull=False
            ).values_list('group_id', flat=True)
            return Group.objects.filter(
                id__in=group_ids, status='active'
            ).distinct()
    ...
    def add_student(self, request, pk=None):
        ...
        membership, created = GroupMembership.objects.get_or_create(group=group,student=student)   
